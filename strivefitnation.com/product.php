...
for (i = 0; i < products.data.length; i++) {
        if (products.data[i].offer == 'main') {
			document.querySelector('#qty_dropdown').innerHTML += `<option value="${products.data[i].id}">${products.data[i].name} - $${products.data[i].price[0]}/Month </option>`
		}
}		
...
$("#qty_dropdown").change(function(e){
	$("#chosenPlan").html('');
    $("#chosenPlan").html(products.data[index].type[i])
    $("#chosenPrice").html(`$${products.data[index].price[0].toFixed(2)}`)
	
	$("#chosenPlan").html('');
    $("#chosenPlan").html(products.data[this.value-1].name)
    $("#chosenPrice").html(`$${products.data[this.value-1].price[0].toFixed(2)}`)
});
...
$("#addtocart").click(function(e){
	...
	if ( $('#qty_dropdown').val() == '' ||  $('#qty_dropdown').val() == null){
		...
		location.href='addtocart.php?reqid='+$('#qty_dropdown').val()+'&reqpid=0&reqqty=0&package=0';
	}
});/*
